...
OUTPUT_DIR = Path("SVM & DT/notebooks")
...
- Linear Support Vector Machine with TF-IDF features
...
'''
        ...
        markdown("## 11. Support Vector Machine"),
        code(
            r'''
linear_svm = Pipeline(
    steps=[
...
Use the comparison table above to report which model performed better on this train/test split. Linear SVM is a strong baseline for sparse TF-IDF text features because it can separate high-dimensional classes effectively. Decision Trees are easier to interpret, but they can overfit sparse text features, so their performance should be compared carefully with the SVM results.
"""
        ),
    ]
